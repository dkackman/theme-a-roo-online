...
export const Banner = ({
  ...
  // Style banner based on theme's mostLike property
  // CSS variables (bg-primary, text-primary-foreground) already adapt to theme
  // but we can add theme-specific enhancements if needed
  const _isDark = currentTheme?.mostLike === "dark";
...
export const BannerIcon = ({
  ...
  const { currentTheme } = useSimpleTheme();
  // Icon styling adapts via CSS variables, but mostLike can be used for enhancements
  const _isDark = currentTheme?.mostLike === "dark";
...
export const BannerAction = ({
  ...
  const { currentTheme } = useSimpleTheme();
  // Button styling adapts via CSS variables and variant prop
  const _isDark = currentTheme?.mostLike === "dark";

  return (
    <Button
      className={cn(
        "shrink-0 bg-transparent text-primary-foreground hover:bg-background/10",
        className
      )}
      size={size}
      variant={variant}
